...
export default function Roulette({ userId, user, setUser }){
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      {/* Countdown в левом верхнем углу */}
      <div className="absolute top-4 left-4 z-10">
        {getCountdownDisplay() && (
          <div className="glass-card px-3 py-1 rounded-lg">
            <div className="text-xs text-gray-400">Таймер</div>
            <div className="text-sm neon-text font-bold">{getCountdownDisplay()}</div>
          </div>
        )}
      </div>

      {/* Колесо рулетки - увеличено и поднято */}
      <div className="flex-none flex justify-center items-center pt-6 pb-4">
        <div className="relative w-80 h-80 sm:w-96 sm:h-96 rounded-full glass-card overflow-hidden">
          ...
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
              <svg width="50" height="50" viewBox="0 0 50 50" fill="none" xmlns="http://www.w3.org/2000/svg" className="-mt-40">
                <path d="M25 0 L35 20 L15 20 Z" fill="#fff" />
              </svg>
            </div>
          </div>
          ...
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
            {bets.length === 0 ? (
              ...
              <div className="text-center">
                <p className="text-3xl sm:text-4xl neon-text font-bold">{formatTon(totalBet)}</p>
                <Ton className="w-6 h-6 mx-auto mt-1" />
              </div>
            )}
          </div>
        </div>
